...
**Write the whole label, not the bare adjective.** This is the single most
common way to get a field wrong here. The colour of the hair is `blonde hair`,
not `blonde`. The colour of the eyes is `brown eyes`, not `brown`. The skin is
`dark skin`, not `dark`. A bare adjective is ambiguous about what it is an
adjective *of*, and it maps onto nothing. Carry the noun with it every time,
even though the field name already says which noun it is.
...
7. **Do not repeat yourself.** Each phrase appears once in a field. A field with
   the same phrase in it twice is a field that has gone wrong, and no field here
   needs more than about four phrases.
...
skin_ancestry    []
hair_colour      ["dark brown hair"]
hair_silhouette  ["shoulder length", "wavy hair"]
eye_colour       ["brown eyes"]
eyebrows         []
marks            []
...
Note the register of the colours, too. `dark brown hair` and `brown eyes` carry
their noun; `dark brown` and `brown` on their own would name nothing.
...
The sixteen fields, each holding a list of phrases. Nothing before it and
nothing after it — no preamble, no reasoning, no commentary on your own answer.
