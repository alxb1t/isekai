...
### Requirement: A run is identified by its photograph's content

The system SHALL identify a run by a content hash of the photograph plus a readable slug derived from
its filename stem, and SHALL treat a second upload of the same bytes as a continuation of the same run
rather than a new one.

Two different photographs can share a filename, and a bare hash cannot be read in a directory listing.
Deriving the id from content is also what makes re-uploading a resume rather than a duplicate: the same
bytes reach the same directory, and the artifacts already there decide what is left to do.

#### Scenario: the id combines a content hash and a readable slug
- **Key:** `run-directory:identity:id-is-hash-and-slug`
- **Layers:** unit
- **WHEN** a run is created for a photograph
- **THEN** its id begins with a prefix of the photograph's SHA-256 and continues with a sanitised
  filename stem
- **AND** the full digest is recorded in the run's frame

#### Scenario: two photographs sharing a filename get different runs
- **Key:** `run-directory:identity:same-name-different-bytes-differ`
- **Layers:** unit
- **WHEN** two photographs with identical filenames and different bytes are each given a run
- **THEN** the two ids differ
- **AND** neither run's artifacts are visible to the other

#### Scenario: the same photograph twice is one run
- **Key:** `run-directory:identity:same-bytes-resume-the-same-run`
- **Layers:** unit
- **WHEN** a photograph is offered to a command and a run for those bytes already exists
