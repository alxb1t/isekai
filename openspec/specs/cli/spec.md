...
### Requirement: The pipeline surface is a second entry point with subcommands

The system SHALL expose the staged pipeline as subcommands of a module entry point, separate from the
single-command render surface, and SHALL leave that existing surface and its flags unchanged.

A pipeline that stops for a human cannot have a one-command surface, so it needs verbs. Adding them to
the existing parser would change the entry point the current path depends on; a separate module adds
the new surface without touching the proven one. Subcommands under one parser rather than several
scripts keeps argument handling in one place and the stdlib-only guard to a single target.
...
- **AND** the existing single-command render surface is unaffected
