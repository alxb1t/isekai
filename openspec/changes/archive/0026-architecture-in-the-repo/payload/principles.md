...
how it fails. It has no effect outside that contract.** It earns its place if something else could use
...
a rule about the work itself, such as how a failure is classified or what state a sheet is in, belongs to
...
layer, modules may import each other, but never in a cycle. A layer's `__init__.py` holds only a
...
cause is a named refusal; the batch collects them and reports them together at the end. Nothing is
guessed to avoid a refusal — a value that cannot be read or derived refuses its input rather than
...
searching or guessing. What cannot differ between flows is a constant in code that says it is not a
...
moving tag is not a pin. A flow never changes silently: a variant gets a new identifier and the two are
